...
GMModel = fitgmdist(C_spatial,NumClusters,'RegularizationValue',0.01,'Options',options);
centers = GMModel.mu;%cluster centers

posteriory = posterior(GMModel, C_spatial);%possibility matrix, could change C_spatial to other dataset
[maxGMM, clusters] = max(posteriory,[],2);%assign clusters
%or use the trained model to predict new data
%idx = cluster(GMModel, newData);
