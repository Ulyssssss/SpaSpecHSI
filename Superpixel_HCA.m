...
YLAB = pdist(C_spatial,"cosine");%distance measure
...
[~,T] = dendrogram(ZLAB,NumClusters);%plot the dendrogram and save the cluster matrix T
T20 = cluster(ZLAB,"maxclust",NumClusters);% "maxclust" to take all of them or directly specify a threshold with "cutoff"

%compute cluster centers
centers = zeros(NumClusters,bands);
for i = 1:NumClusters
    clusteri = C_spatial(T20 == i,:);
    centers(i,:) = mean(clusteri);
end
...
for i = 1:NLAB
    idx = LLAB == i;
    seg20(idx) = T20(i);%substitute superpixel belongingness to cluster membership
end
...
[colormapValues,XYZ]=HSI2RGB(wavelength,centers,NumClusters,1,65,0.01);
...
%     fileName = sprintf('cluster%d_HCA.png', i);
